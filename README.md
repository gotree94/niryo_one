# Niryo One

https://docs.niryo.com/robots/niryo-one/niryostudio-niryoone/

(Niryo One : [https://niryo.com](https://niryo.com/?utm_source=github))

![Niryo One image](https://niryo.com/wp-content/uploads/2018/09/niryo_one_with_gripper1.jpg)

This repository contains STL files to print Niryo One with a 3D printer.

Note : **You need at least a 200mm\*200mm\*200mm 3D printer to be able to print all the parts.**

You can find Niryo One ROS stack here : https://github.com/NiryoRobotics/niryo_one_ros 

Enjoy !

---

Note : if you have received your Niryo One before September 2018, the files related to your robot are located in the "Archive (v 1.1.0)" folder.

```
# 1. git 저장소 초기화
git init

# 2. 사용자 정보 설정 (GitHub 계정 정보로)
git config --global user.email "your-email@example.com"
git config --global user.name "gotree94"

# 3. 원격 저장소 연결
git remote add origin https://github.com/gotree94/niryo_one.git

# 4. 모든 파일 스테이징
git add .

# 5. 커밋
git commit -m "Initial commit"

# 6. 브랜치 이름을 main으로 변경
git branch -M main

# 7. GitHub에 푸시
git push -f origin main
```

* https://docs.niryo.com/robots/

* STEP 0 Unboxing : https://youtu.be/ijinVVn0aUE
* STEP 1 Base : https://youtu.be/3f-Ecz0IYLY
* STEP 2 Shoulder : https://youtu.be/f9BH28SffIw
* STEP 3 Elbow : https://youtu.be/r_z3i4-z8CU
* STEP 4 Wirst : https://youtu.be/w52qcxzaf_0
* STEP 5 Drawer : https://youtu.be/ym8SnfpQdpg


